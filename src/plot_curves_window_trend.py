...
def curves(start, county, save_plot=False, update=Trues):
    '''
    ...
    res_trend = load_pred_window(start, n_weeks, trend=True)
    prediction_samples_trend = np.reshape(
        res_trend['y'], (res_trend['y'].shape[0],  -1, 412)
        )
    prediction_samples_trend_exp = np.reshape(
        res_trend['μ'], (res_trend['μ'].shape[0],  -1, 412)
        )
    prediction_samples_trend = prediction_samples_trend[:,:n_days,:]
    prediction_samples_trend_exp = prediction_samples_trend_exp[:,:n_days,:]
    ...
    if save_plot:
        ...
        plt.savefig(
            "../figures/{}_{}_{}/curve_trend_{}.png".format(
                year, month, day,countyByName[county]
                ), 
            dpi=200
            )
