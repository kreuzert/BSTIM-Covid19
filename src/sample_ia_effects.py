...
num_sample = nums_sample[int(os.environ["SGE_TASK_ID"]) - 1]

filename = "../data/ia_effect_samples/{}_{}.pkl".format(disease, num_sample)

print("Running task {} - disease: {} - sample: {}\nWill create file {}"
    .format(int(os.environ["SGE_TASK_ID"]), disease, num_sample, filename))
...
samp = IAEffectSampler(
    data, times, locs, temporal_bfs, spatial_bfs, num_tps=10, time_horizon=5
    )
